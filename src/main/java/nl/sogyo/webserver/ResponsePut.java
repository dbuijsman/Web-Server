...
public class ResponsePut implements Response{
    private HttpStatusCode status;
    private Map<String, String> customHeaders = new HashMap<>();
    private ZonedDateTime date;
    private byte[] content;
    ResponsePut(String resourcePath, String content){
        this.date = ZonedDateTime.now();
        try {
            File requestedFile = new File("src/main/resources" + resourcePath);
            if (requestedFile.exists()) {
                File newFile = createFile(resourcePath, content);
                if (requestedFile.delete() && newFile.renameTo(requestedFile)) {
                    this.status = HttpStatusCode.OK;
                    this.content = ("Modified resource: " + resourcePath).getBytes();
                } else {
                    throw new IOException();
                }
            } else {
                this.status = HttpStatusCode.NotFound;
                this.content = ("Resource" + resourcePath + "don't exist.").getBytes();
            }
        } catch(IOException e){
            this.status = HttpStatusCode.ServerError;
            this.content = ("Can't change resource: " + resourcePath).getBytes();
        } finally {
    ...
    private File createFile(String resourcePath, String content) throws IOException{
        Charset utf8 = StandardCharsets.UTF_8;
        StringBuilder tempResourcePath = new StringBuilder(resourcePath).insert(resourcePath.indexOf("."),"temp");
        try (Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("src/main/resources" + tempResourcePath), utf8))) {
            writer.write(content);
            return new File("src/main/resources" + resourcePath);
        } catch (IOException e) {
            throw new IOException();
        }
    }
